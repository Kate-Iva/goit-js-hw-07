...
const images = [
  ...
  const insertedPictures = images
  .map(
    imageObject =>
      `<li class='gallery-item'><img class='gallery-item-image' src='${imageObject.url}' alt='${imageObject.alt}' /></li>`
  )
  .join('');

galleryContainer.insertAdjacentHTML('beforeend', insertedPictures);